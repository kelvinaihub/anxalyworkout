...
import { WorkoutPlan, User, Exercise, GeneratedExercise, WorkoutSession } from '../../types.ts';
...
const InstructionModal: React.FC<{ exercise: UnifiedExercise, onClose: () => void }> = ({ exercise, onClose }) => {
    const instructions = 'instructions' in exercise ? exercise.instructions : ('notes' in exercise ? exercise.notes : 'No instructions available.');
...
// --- MAIN ACTIVE WORKOUT SCREEN COMPONENT ---

const ActiveWorkoutScreen: React.FC<{ plan: WorkoutPlan; currentUser: User | null; onFinish: () => void; onSetFooterVisibility: (isVisible: boolean) => void; }> = ({ plan, currentUser, onFinish, onSetFooterVisibility }) => {
  ...
  const allExercises = useMemo(() => {
    // Definitive fix: Ensure plan and plan.exercises are valid arrays to prevent crashes.
  ...
  const totalSets = useMemo(() => (currentExercise && 'sets' in currentExercise ? currentExercise.sets : 1), [currentExercise]);
  const isRepBased = useMemo(() => currentExercise && currentExercise.reps.toLowerCase().includes('rep'), [currentExercise]);
  ...
  const restTime = useMemo(() => (currentExercise && 'restTime' in currentExercise ? currentExercise.restTime : 30) || 30, [currentExercise]);
  ...
  if (!allExercises || allExercises.length === 0 || !currentExercise) {
    return (
      <div className="fixed inset-0 bg-dark-bg text-white flex flex-col items-center justify-center p-4 text-center">
        ...
        <p className="mt-2 text-dark-text-secondary">This workout plan contains no exercises and cannot be started.</p>
    ...
    mainTimerRef.current = window.setInterval(() => {
      ...
      switch(workoutPhase) {
        ...
        case 'working':
          if(isRepBased) {
              setRepPacer(prev => {
                  if (!prev) return { rep: 1, phase: 'lift', progress: 0 };
                  const newProgress = prev.progress + (100 / 2);
                  if (newProgress >= 100) {
                      if (prev.phase === 'lift') {
                          playBeep(660, 0.1);
                          return { ...prev, phase: 'lower', progress: 0 };
                      }
                      if (prev.rep >= totalReps) {
                          playBeep(1200, 0.2, 'square');
                      ...
                      return { rep: prev.rep + 1, phase: 'lift', progress: 0 };
                  }
            ...
            setTimeLeft(prev => {
              ...
              if (newTimeLeft <= 3) { playBeep(1000, 0.1, 'sine');
              }
              return newTimeLeft;
            });
          }
        ...
        case 'resting':
          setTimeLeft(prev => {
            ...
            if (newTimeLeft <= 3) { playBeep(880, 0.1, 'sine'); }
            return newTimeLeft;
          });
          break;
      }
  ...
  const handleFinishWorkout = async () => {
    const sessionData: Omit<WorkoutSession, 'id'> = {
        ...
        duration: Math.round(timeElapsed / 60) || 1,
        planNotes: plan.notes,
        caloriesBurned: plan.caloriesBurned,
        completedExercises: allExercises.slice(0, currentIndex + 1).map(ex => ({
            exerciseId: 'id' in ex ? ex.id : ex.name,
            name: ex.name,
            notes: 'notes' in ex ? ex.notes : ('instructions' in ex ? ex.instructions : ''),
            sets: Array.from({ length: ex.sets }, () => ({ reps: 10, weight: 20 }))
        })),
        progressMedia: [],
    };
    try {
        const savedSession = await saveWorkoutSession(sessionData);
        setCompletedWorkout(savedSession);
        setWorkoutPhase('workout_finished');
    } catch (error) {
        console.error("Failed to save workout session:", error);
        setCompletedWorkout({ ...sessionData, id: 'temp-id' });
        setWorkoutPhase('workout_finished');
    }
  };
  ...
  const startSet = () => {
    if (workoutPhase === 'ready') {
      ...
      if (!isRepBased) {
        setTimeLeft(timeBasedDuration);
      } else {
        setRepPacer({ rep: 1, phase: 'lift', progress: 0 });
      }
      setCountdown(3);
  ...
  const handleSpeak = () => {
    if ('speechSynthesis' in window) {
      const text = `${currentExercise.name}. ${isRepBased ? `${currentSet} sets of ${currentExercise.reps}` : `Hold for ${currentExercise.reps}`}.`;
  ...
  const handleRepeatSet = () => {
    if (!isRepBased && (workoutPhase === 'working' || workoutPhase === 'resting')) {
        setTimeLeft(timeBasedDuration);
        if (workoutPhase !== 'working') { setWorkoutPhase('working'); }
    }
  };

  const handleBackPress = () => {
    if (window.confirm('Are you sure you want to end this workout? Your progress will not be saved.')) {
        onFinish();
    }
  };
  ...
  if (workoutPhase === 'workout_finished' && completedWorkout && currentUser) {
    return <WorkoutSummaryScreen user={currentUser} workoutHistory={[completedWorkout]} initialSession={completedWorkout} onDone={onFinish} />;
  }
  ...
  const imageUrl = 'media' in currentExercise && currentExercise.media[0]?.url ? currentExercise.media[0].url : ('imageUrl' in currentExercise ? currentExercise.imageUrl : 'https://picsum.photos/800/1200');

  const renderWorkoutContent = () => {
      switch (workoutPhase) {
          case 'ready':
              return <div className="text-center"><button onClick={startSet} className="bg-white text-black font-bold py-4 px-12 rounded-full text-2xl">Start Set {currentSet}</button></div>;
          case 'countdown':
              return <p className="text-8xl font-bold">{countdown}</p>;
          case 'working':
              return (
                <div className="flex flex-col items-center">
                    {isRepBased && repPacer 
                        ? <RepPacer currentRep={repPacer.rep} totalReps={totalReps} phase={repPacer.phase} progress={repPacer.progress} />
                        : <p className="text-8xl font-bold font-mono tracking-tighter">{timeLeft.toString().padStart(2, '0')}</p>
                    }
                </div>
              );
          case 'resting':
              return (
                <div className="text-center">
                    ...
                    <p className="text-6xl font-mono tracking-tighter">{timeLeft.toString().padStart(2, '0')}</p>
                </div>
              );
          case 'exercise_complete':
              return <div className="text-center"><button onClick={handleNextExercise} className="bg-white text-black font-bold py-4 px-12 rounded-full text-2xl flex items-center">Next Exercise <ChevronRightIcon className="w-6 h-6 ml-2"/></button></div>;
          default:
              return null;
      }
  };
  ...
  const renderFooterControls = () => {
    ...
    return (
      <div className="flex items-center space-x-2">
          <button onClick={handleRepeatSet} disabled={isRepBased || !isTimerActive} className="p-2 rounded-full hover:bg-white/20 disabled:opacity-50">
              <RepeatIcon className="w-6 h-6 text-gray-300"/>
          </button>
          <button onClick={handleTogglePause} disabled={!isTimerActive} className="p-2 rounded-full hover:bg-white/20 disabled:opacity-50">
  ...
  const renderFooterTimer = () => {
    ...
    switch (workoutPhase) {
      case 'countdown':
        ...
        totalDuration = 3;
        progress = totalDuration > 0 ? ((totalDuration - displayTime) / totalDuration) * 100 : 0;
        break;
      case 'working':
        if (!isRepBased) {
          displayTime = timeLeft;
          totalDuration = timeBasedDuration;
          progress = totalDuration > 0 ? ((totalDuration - displayTime) / totalDuration) * 100 : 0;
        } else {
            return <div className="flex-1 px-2" />; // Hide bar for rep-based work
        }
        break;
      case 'resting':
        displayTime = timeLeft;
    ...
    return (
      <div className="flex-1 px-2 flex flex-col justify-center items-center">
        <span className="font-mono text-lg font-bold text-white tracking-wider" style={{ textShadow: '0 1px 3px rgba(0,0,0,0.5)' }}>
          {displayTime.toString().padStart(2, '0')}
        </span>
        <div className="w-full bg-white/20 rounded-full h-3 -mt-2 overflow-hidden">
          <div 
            className="bg-gradient-to-r from-cyan-400 to-blue-500 h-3" 
  ...
  return (
    <div className="fixed inset-0 bg-black text-white flex flex-col font-sans">
      ...
      <header className="relative z-10 p-4 pt-8 md:pt-4 space-y-4">
        <button onClick={handleBackPress} className="p-2 bg-black/30 rounded-full">
            <ArrowLeftIcon className="w-6 h-6"/>
        </button>
        <div className="flex items-center space-x-4">
            <div className="w-24 text-left text-sm font-mono">{Math.floor(timeElapsed/60).toString().padStart(2, '0')}:{(timeElapsed % 60).toString().padStart(2, '0')}</div>
            <div className="flex-1 bg-white/20 h-1.5 rounded-full"><div className="bg-white h-1.5 rounded-full" style={{ width: `${progressOverall}%` }}></div></div>
            <div className="w-24 text-right text-sm font-semibold">{Math.ceil((totalDurationSeconds - timeElapsed)/60)} MIN LEFT</div>
        </div>
      </header>
      ...
      <footer className="relative z-10 w-full p-4 space-y-3">
          <div className="flex items-center space-x-3">
              <p className="flex items-center font-semibold"><HeartIcon className="w-5 h-5 mr-1 text-red-500" /> {Math.round(mockHeartRate)}</p>
              <div className="flex-1 h-2 rounded-full overflow-hidden bg-gradient-to-r from-cyan-500 via-green-500 to-red-500"></div>
              {currentUser && <img src={currentUser.profilePictureUrl} alt={currentUser.name} className="w-8 h-8 rounded-full" />}
          </div>
          <div className="bg-white/10 backdrop-blur-md rounded-xl p-4 text-left">
              <div className="flex justify-between items-start">
                  <div>
                    <p className="text-xs font-semibold text-gray-300">{'equipment' in currentExercise ? currentExercise.equipment : 'Bodyweight'}</p>
              ...
              <div className="flex items-center space-x-2 mt-2">
                  ...
                  <button onClick={() => setIsSetSelectorOpen(true)} className="text-center leading-tight p-2 rounded-md hover:bg-white/10 transition-colors">
                    <p className="font-semibold text-xs text-gray-400">SET</p>
                    <p className="font-bold text-lg">{currentSet}/{totalSets}</p>
                  </button>
                  {renderFooterTimer()}
