...
exports.dailyWorkoutGenerator = void 0;
...
const genAI = new genai_1.GoogleGenAI({ apiKey: process.env.GEMINI_API_KEY || 'AIzaSyCEx-ewOOEYgyL2u2mspVEHdOSg95m_K00' });
// Helper function to generate workout using Gemini
const generateWorkoutForUser = async (user) => {
    const prompt = `
...
exports.dailyWorkoutGenerator = functions.pubsub
    .schedule('0 0 * * *')
    .timeZone('Asia/Ho_Chi_Minh')
    .onRun(async (context) => {
    console.log('Starting Daily Workout Generation Job...');
    try {
        // 1. Fetch all users, ordered by ID (or creation time)
        // This ensures "Admin (ID 1)" is processed first, then others sequentially.
        const usersSnapshot = await db.collection('users')
            .orderBy('id', 'asc') // Or 'createdAt'
            .get();
        ...
        for (const user of users) {
            ...
            if (workoutPlan) {
                // Save to Firestore
                await planRef.set(Object.assign(Object.assign({}, workoutPlan), { createdAt: admin.firestore.FieldValue.serverTimestamp(), date: today, validUntil: admin.firestore.Timestamp.fromMillis(Date.now() + 24 * 60 * 60 * 1000) // Valid for 24h
                 }));
